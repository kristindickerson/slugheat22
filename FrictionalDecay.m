...
if LowestK < 0
    ...
    if ~isempty(ShiftedRelativeDepths)
             ...
             Currentk(all(Kappa<0))=A + B*RelativeDepths(all(Kappa<0)) + C*RelativeDepths(all(Kappa<0)).^2 ...
                               + D*exp(E*RelativeDepths(all(Kappa<0))) + F*log10(RelativeDepths(all(Kappa<0)));
    end
    Kappa = 1e-6*Currentk(SensorsToUse)./(HC(1) ...
        - HC(2)*Currentk(SensorsToUse) ...
        + HC(3)*Currentk(SensorsToUse).^2);
...
try
    ...
    uialert(figure_Main, ['\bfError! Program failure. ' newline newline ...
        'Check the following data to ensure program feasibility:' newline newline ...
        '- Ensure all input parameters are correct.' newline newline ...
        '- Ensure bottom water is correct.' newline newline ...
        '- Ensure bad data is ignored. To ignore spurious data, you may need to move the timing of events such as the start of penetration, heat pulse, or end of penetration. Use the text boxes in the command window to left.' newline newline ...
        '- If all paramters are correct and data time windows are all correct, you may need to remove a sensor entirely. If a sensor did not penetrate and was recording bottom water temperature, the program could fail. This sensor has no measurements in the required \tau range: ' num2str(whichsens) newline 'You may need to discard all data from this sensor to proceed.' newline newline  ...
        '- Finally, ensure data is within required \tau range. Check heat pulse power, as this affects \tau. ' ...
        'If the heat pulse power is too low, you must raise power or ignore the heat pulse reduction. ' ...
