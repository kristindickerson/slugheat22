...
function [h_axHPTempvTime, ...
    ...
    % ---------------------------------------------
        ...
        ymaxAll = max(ymax);
            ...
            % ---------------------------
            xlabel(axes_CTempvCtime, 'Shifted Time (s)', 'verticalalignment','top', ...
            'fontsize',16)
            ylabel(axes_CTempvCtime, 'Temperature relative to T_{\infty} ( ^oC)', 'verticalalignment',...
                'bottom', 'fontsize',16)
    ...
    % -------------------------------------------------------------
        ...
        % --------------------------
            xlabel(axes_MisfitvTimeShift, 'Time Shifts (s)', ...
                ...
                'fontsize',16)
            ylabel(axes_MisfitvTimeShift, 'Relative deviation for k_{slope} (W/m ^oC)', ...
        ...
        axes_MisfitvTimeShift.XTick = minshift:10:maxshift;

        % Label for lines showing minimum misfit during time shifts 
        % (point used for subsequent analyses)
        % --------------------------------------------------------
        lgn = [h_axHPRMS(SensorsToUse(1)), h_axHPRMSLine(SensorsToUse(1))];
    ...
    % ------------------------------------------------
        ...
        % --------------------------
          xlabel(axes_CTempv1_CTime, '1/Time (s^{-1})', ...
            'verticalalignment','top', ...
            'fontsize',16)
          ylabel(axes_CTempv1_CTime, 'Temperature relative to T_{\infty} ( ^oC)', ...
            'verticalalignment','bottom', ...
            'fontsize',16)
    ...
    % --------------------------------------------
        ...
        minshift = min(min(TimeShifts(:,:)));
        maxshift = max(max(TimeShifts(:,:)));
        axes_TempvTimeShift.XTick = minshift:10:maxshift;
        ...
        % (point used for subsequent analyses)
        % --------------------------------------------------------
        lgn = [h_axHPTempvTimeShift(SensorsToUse(1)), h_axHPTempvTimeShiftBestFit(SensorsToUse(1))];
